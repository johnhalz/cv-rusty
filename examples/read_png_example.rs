...
fn main() {
    // Example usage of reading a PNG image
    println!("CV Rusty - PNG Reader Example\n");

    // Try to read a PNG file from the command line argument
    let args: Vec<String> = std::env::args().collect();

    if args.len() < 2 {
        eprintln!("Usage: {} <path_to_png_file>", args[0]);
        eprintln!("\nExample: {} photo.png", args[0]);
        std::process::exit(1);
    }

    let png_path = &args[1];
    println!("Reading PNG file: {}", png_path);

    match read_png(png_path) {
        Ok(image) => {
